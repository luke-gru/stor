...
static tbl_t *db_table(db_t *db, int i) {
    ASSERT(i >= 0 && i < db->db_meta.mt_tbls.length);
    return db->db_meta.mt_tbls.data[i];
}

static col_t *db_col(tbl_t *tbl, int i) {
...
static int db_flush_meta(db_t *db) {
    if (db->db_fd <= 0) {
        return -1;
    }
    if (!db->db_mt_dirty) {
...
static const char *coltype_str(coltype_t coltype) {
    switch(coltype) {
    case COLTYPE_INT:
        return "int";
    case COLTYPE_CHAR:
...
static int db_add_table(db_t *db, const char *tblname, const char *colinfo, dberr_t *dberr) {
...
static blkh_t *db_load_blk(db_t *db, uint16_t num) {
    int i = 0;
    blkh_t *cachedblk;
...
static int db_add_record(db_t *db, const char *tblname, const char *rowvals, blkh_t **blkh_out, bool flushblk, dberr_t *dberr) {
    const char *valsep = ",";
    tbl_t *cur = NULL;
    tbl_t *tbl = NULL;
    int i = 0;
...
/*static bool db_update_record(db_t *db, blkh_t *blk, tbl_t *tbl, rec_t *rec, vec_dbupdval_t *updates) {*/
/*}*/

static int db_parse_srchcrit(db_t *db, tbl_t *tbl, const char *srchcrit_str, vec_dbsrchcrit_t *vsearch_crit, dberr_t *dberr) {
...
static tbl_t *db_find_table(db_t *db, const char *tblname) {
    tbl_t *curtbl = NULL;
    int i = 0;
...
static int db_find_records(db_t *db, tbl_t *tbl, vec_dbsrchcrit_t *vsearch_crit, srchopt_t *options, vec_recinfo_t *vrecinfo_out, dberr_t *dberr) {
...
static int db_update_records(db_t *db, vec_recinfo_t *vrecinfo, vec_dbsrchcrit_t *vupdate_info, dberr_t *dberr) {
    recinfo_t *recinfo_p;
    int i = 0;
    rec_t *rec;
    blkh_t *blk;
...
static int db_delete_records(db_t *db, vec_recinfo_t *vrecinfo, dberr_t *dberr) {
    recinfo_t *recinfo_p;
    int i = 0;
    rec_t *rec;
    blkh_t *blk;
...
static int db_find_record(db_t *db, tbl_t *tbl, vec_dbsrchcrit_t *vsearch_crit, recinfo_t *recinfo_out, dberr_t *dberr) {
    srchopt_t opts;
    opts.limit = 1;
